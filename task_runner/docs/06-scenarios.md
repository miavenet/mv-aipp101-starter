...
| RUN-32 | an accepted producer's gate names one of its own outputs (behind a glob in `writes`), and a later `replan` changes nothing about it **(B9)** | the replan is not refused: the protection the expansion derives from gate words once the output is tracked is left out of the definition comparison | `run: a gate naming the task's own output is not a change` |
...
| PAUSE-02 | `runner pause RUN --now` while an agent call is in flight | the process named in the run lock (not any process by name) gets SIGTERM, stops its children and exits; the lock is released; STATUS.md is refreshed at the interrupt, so it lists the interrupted operation and the `resume` that reconciles it; the call in flight is lost, and `resume` reconciles it, runs the attempt again, and the line is gone | `pause: --now` |
...
| REC-17 | `STATUS.md` is written while a runner holds the run lock and a call or command is open | the open operation appears under "In flight" and not as "interrupted"; the "were interrupted" line is written only when no live runner holds the lock, which is when an open intent is one left behind | `rec: an open operation is interrupted only when no runner is working` |
...
| BUD-07 | a call ends without the provider's final event: the runner was stopped with `pause --now`, the call timed out, or the orphan was killed on `resume` | the call's usage is read from the provider's own record (Claude transcript, Codex rollout), rows after the call began only, once per response; it is unpriced usage with `usage_source: provider-record`, not an unknown call; when no record is found the call stays unknown; while a call runs, its "In flight" line in STATUS.md shows the tokens so far from the same record | `bud: provider record`, `agent: provider record` |
